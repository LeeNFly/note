...
// 在入口文件中:
//    1. 引入了vue (vue的js文件), App.vue(单文件组件, 一个局部组件, 也叫根组件, 整个项目的根组件), 以及路由;
...
// App又称为整个项目的根组件, 将来组件都是通过根组件的如有出口展示渲染到index.html这个文件中的

// 说明: 根据以上思路可知, 我们将来访问项目, 一定都是先访问模板文件(index.html) (根页面), 然后在访问模板文件(根页面)的基础上, 加上#锚点, 让路由监听到变化, 自动在index.html(根页面)中展示不同的组件
//       实际上我们访问的就只有一个页面 index.html (根页面), 单页面应用, 就是index.html单页面
//       我们在根页面index.html的#app (el标签) 标签外写的内容 也会被展示在页面上 (访问的就是这个根页面index.html)
