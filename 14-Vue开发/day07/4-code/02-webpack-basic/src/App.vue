...
<template>
  <!-- 模板 (该组件页面部分)
          ...
          不能为空
          该组件实例的管理范围为本组件的模板, 在模板中可以使用所有vue特性, 也拥有所有的vue特性, 比如数据驱动, 双向绑定, 插值表达式, 指令, 自动渲染等等. 
   -->
  <div>
...
// JS逻辑代码  (该组件逻辑部分)
// script标签内可以为空
// script标签内也可以写js代码 比如import某个模块
// 在单文件组件中也可以使用其他组件
// 在单文件组件中也可以导入其他组件(vue实例), 然后在单文件组件中使用(在模板中使用), 由于单文件组件是局部组件, 
