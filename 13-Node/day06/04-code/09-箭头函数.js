...
// 更确切的说是, 原来的匿名函数声明方式, 都可以使用箭头函数来声明, 箭头函数是一种声明(创建)函数的方式, 类似于匿名函数的声明函数方式, 返回函数的引用地址
// 原来使用匿名函数的地方, 都可以使用箭头函数来替代
// function (形参列表) { 函数体 }  改成使用箭头函数 (形参列表) => { 函数体 }
// (形参列表) => { 函数体 } 相当于 ★ 声明(创建)了一个匿名函数, 开辟一块新的空间存储, 返回函数的引用地址, 且声明函数是不会执行函数体的, 需要调用才会执行函数体
// 其他同ES5函数一致, 比如参数可有可无, 形参的命名规范要符合函数的命名规范及形参总结、返回值|变量查找、函数作用域及作用域等等. 因为本质上就是函数
// 形参的声明方式默认使用var
