...
// 4. let声明的变量只能存储一个值, 可以存储所有类型数据, 可以多次赋值, 可以接收任意类型的赋值, 数据类型自带改变, 后面的赋值会覆盖前面存储的值
// 5. 其他和var一致
...
// 3. const声明的变量不能被修改值, const声明的变量只能存储一个值, 值可以是任意类型
...
//    特殊: 一个函数内部使用let或const声明变量, 其被多次调用, 是被允许的, 不算let和const重复声明, 且在当前作用域中, 后声明的会覆盖之前声明的变量
