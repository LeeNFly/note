    ...
    <div className="App">
      <HashRouter>
        ...
        <div>
          ...
          {/* URL中#锚点部分匹配的路由规则对应的那个组件中render方法返回的编译后的jsx会替换覆盖展示(拼接)到Route处, 并且该部分jsx由该组件内部数据自己维护 */}
          {/* Route中只会展示当前URL中#锚点部分匹配的路由规则Route对应的组件(jsx), 对于URL中#锚点部分没有匹配Route, 则Route中什么都不展示, 若之前有展示过, 则会清除*/}
          {/* 路由会自动监听地址栏URL中#锚点部分, 只要改变, 就会重新匹配路由规则Route, 然后只将当前URL#锚点匹配的那个路由规则对应的组件中render方法返回的编译后的jsx覆盖替换展示(拼接)到Route处 (会将这次匹配的组件render方法返回的jsx替换掉上次匹配组件render方法返回的jsx), 并且该部分jsx由组件内部数据自己维护*/}
