...
$(function() {
  ...
  /*
  * 2. 登陆功能
  *    表单校验插件会在提交表单(type: submit)时, 根据配置的校验信息对form内的相关表单元素进行校验 (一定是form表单中的submit按钮才可以触发配置的表单校验)
  *    (1) 表单form校验成功, 默认就提交表单, 会发生页面跳转,
  *        我们需要注册表单校验成功事件, 阻止默认的表单提交, 通过ajax进行发送请求
  *    (2) 表单form校验失败, 则不会进行提交表单, 配置插件提示用户即可 (会将配置的校验提示信息自动提示给用户)
  *
  * */
