...
> bootstrap-validator插件会在表单提交的时候根据配置的校验信息对form内的相关表单元素进行校验，如果校验成功了，表单会继续提交，但是如果校验失败了，就会阻止表单的提交。
>
> 表单校验插件会在表单变化时, 自动更新校验状态
>
> 一定是form表单中的submit按钮才可以触发配置的表单校验
...
// 1. 传true, 重置form表单内所有的表单元素value值以及校验状态(隐藏所有的错误提示和图标)
// 2. 传false, 只重置校验状态(隐藏所有的错误提示和图标), 默认false
```

### 更新字段的状态

BootstrapValidator在用户输入内容的时候，会做校验，当调用bootstrap的插件的方法可以手动会改变字段值的状态。可以使用`updateStatus(field, status, validatorName)`方法更新字段的校验状态
